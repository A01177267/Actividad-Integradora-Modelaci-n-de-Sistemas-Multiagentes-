...
class CrossRoad(Model):
    ...
    def step(self):
        ...
        if self.traffic_counter < self.traffic_time-2:
            self.traffic_counter += 1
        elif self.traffic_counter < self.traffic_time:
            for i, direction in enumerate(Car.DIRECTIONS):
                if self.traffic_lights[direction].colour == 'green':
                    self.traffic_lights[direction].colour = 'yellow'
            self.traffic_counter +=1
            ...
            for i, direction in enumerate(Car.DIRECTIONS):
                ...
                if self.traffic_lights[direction].colour == 'yellow':
                    self.traffic_counter = 0
